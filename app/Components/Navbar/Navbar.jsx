...
const Navbar = ({scrollThreshold = 50 }) => {
  ...
  return (
    <>
    <div className={`flex fixed w-full justify-between items-center max-sm:px-2 max-sm:py-3 sm:px-3 sm:py-4  md:px-4 md:py-4  lg:mx-auto lg:ml-auto lg:mr-auto z-50 ${scrolled ? 'bg-[white] shadow-md shadow-black/40 transition ease-linear' : ''}`}>
...
<ButtonDark btnText={"Register"} hrefProp={"/register"}/>
    ...
    <div className={nav ? ' bg-[#C5DCFF] w-[60%] h-screen fixed top-0 right-0 left-0 bottom-0  duration-1000  z-50 lg:hidden'  : 'bg-[#C5DCFF] w-[60%] h-screen fixed top-0 right-0 left-[-100%] bottom-0 z-50  duration-1000'}>
 ...
 <Link href={"/"} className='lg:hidden md:text-base  max-sm:text-sm font-bold'>MediLab Hospital</Link>
        </div>
